...
- Fail closed when the CSCA cache is missing, future-dated, or older than the configured offline window, and prevent placeholder network adapters from advancing freshness without authenticated trust material.

### Fixed

- Make passport and DTC verification consume the same governed core trust store populated by signed package import.
- Exclude expired, unauthenticated, stale, and certificate-invalid CSCA records from verification.
